            ...
            <body>
                ...
                <div className={ `container-fluid ${styles.containerFluid}` }>
                    <div className={styles.containerPeca}>
                        <p><Link href={ `` }>Home</Link> / <Link href={ `` }>Monte seu PC</Link> / <Link href={ `` }>Placas-mãe</Link> / <span>Placa-mãe {`${data[0].fabricante} ${data[0].modelo} ${data[0].frequencia} GHz ${data[0].nucleos}-Core` }</span></p>
                        <h1>Placa-mãe {`${data[0].fabricante} ${data[0].modelo} ${data[0].frequencia} GHz ${data[0].nucleos}-Core` }</h1>
...
export const getStaticPaths: GetStaticPaths = async () => {
    const response = await fetch(`https://api-caiomoizes.vercel.app/placas-mae`);
...
export const getStaticProps: GetStaticProps = async (context) => {
    ...
    // const response = await fetch(`http://localhost:5000/placas-mae/${url}`);
    const response = await fetch(`https://api-caiomoizes.vercel.app/placas-mae/${url}`);
    const data = await response.json();
